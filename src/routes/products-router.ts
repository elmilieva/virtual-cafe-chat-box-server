...
router.get("/", (req, res, next) =>
  (<ProductRepository>req.app.locals.productRepo)
    .findAll()
...
router.post("/", async (req, res, next) => {
  ...
  try {
    const found = await (<ProductRepository>(
      req.app.locals.productRepo
    )).findByName(newProduct.name);
    if (found) {
      throw new AppError(400, `Name already taken: '${newProduct.name}'.`);
    }

    // Create new Product
    const created = await (<ProductRepository>req.app.locals.productRepo).add(
      newProduct
    );
...
router.put("/:id", async function (req, res, next) {
  ...
  try {
    ...
    const updated = await (<ProductRepository>req.app.locals.productRepo).edit(
      product
    );
...
router.delete("/:id", async function (req, res, next) {
  ...
  try {
    ...
    const deleted = await (<ProductRepository>(
      req.app.locals.productRepo
