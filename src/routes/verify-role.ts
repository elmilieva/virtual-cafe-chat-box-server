...
export function verifyRole(roles: Role[]) {
  return async function (req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const user = await (<UserRepository>req.app.locals.userRepo).findById(
        userId
      );
      if (!user) {
      ...
      if (roles.some((role) => user.roles.some((r) => r === role))) {
        // if the interection between required and actual user roles is not empty
        ...
        next({
          status: 403,
          message: `Not enough privilegies for this operation.`,
        }); //Error
      }
    } catch (err) {
      next(err);
