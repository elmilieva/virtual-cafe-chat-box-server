...
router.get("/:id", async (req, res, next) => {
  ...
  // find user
  try {
    const found = await (<UserRepository>req.app.locals.userRepo).findById(
      req.params.id
    );
...
router.put("/:id", async function (req, res, next) {
  ...
  try {
    ...
    // _id and username are unmodifiable
    user._id = found._id;
...
router.delete("/:id", async function (req, res, next) {
  ...
  try {
    const userId = req.params.id;
    const deleted = await (<UserRepository>req.app.locals.userRepo).deleteById(
      userId
    );
